...
Dagster's core abstractions are **[ops](/concepts/solids-pipelines/solids)** and **[jobs](/concepts/solids-pipelines/pipelines)**.

**[Ops](/concepts/solids-pipelines/solids)** are individual units of computation that we wire together to form **[jobs](/concepts/solids-pipelines/pipelines)**.
...
object="OpExecutionContext" displayText="context" /> in which it executes (provided by the Dagster framework as the first argument to solids who specify it), and also returns no outputs. Don't worry, we'll soon encounter solids that are much more dynamic.
...
displayText="@job" />, we use function calls to indicate the dependency structure of the solids making up the job. Here, we indicate that the execution of `hello_cereal` doesn't depend on any other solids by calling it with no arguments.
